...
export function reportHistoryService(app) {
    const listReportHistories = async (isUpdate, ts, ack, body, client) => {
        ...
        try {
            ...
            console.log({ filterReport, filterConversation, filterReportUser, filterStartDate, filterEndDate })
            const filters = { creator: user }
            if (filterReport && filterReport != 'all') filters.reportConfigId = filterReport
            if (filterConversation) filters.conversations = filterConversation
            if (filterReportUser) filters.reportUsers = filterReportUser
            if (filterStartDate || filterEndDate) {
                filters.sentTime = {}
                if (filterStartDate) filters.sentTime.$gte = filterStartDate
                if (filterEndDate) filters.sentTime.$lte = filterEndDate
            }
            ...
            if (isUpdate) {
                ...
                console.log(`${performance.now() - t0} cost`)
            } else {
    ...
    app.action('action_choose_report_history_item', async({ ack, body, payload, client }) => {
        ...
        console.log('choose report id ' + selectedId)
