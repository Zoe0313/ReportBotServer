...
export function manage_report_service(app) {
    ...
    app.action('action_choose_report_item', async({ ack, body, payload, say, client }) => {
        ...
        console.log('choose report id ' + selected)
    ...
    app.action({
        ...
        console.log(`display remove report confirm modal, id: ${id}`)
        if (!id) return
        try {
    ...
    app.view('view_remove_confirmation', async({ ack, body, payload, client }) => {
        ...
        console.log(`remove report, id: ${id} ts: ${ts}`)
        if (!id) return
        try {
    ...
    app.action('action_cancel_next_report', async({ ack, body, payload, client }) => {
        ...
        const state = await getState(ts)
        const id = state.selectedId
        console.log(`cancel next report sending, id: ${id} ts: ${ts}`)
        if (!id) return
        try {
    ...
    const updateModal = async ({ ack, payload, body, client }) => {
        ...
        const state = await getState(ts)
        const id = state.selectedId
        console.log('change repeat type id ' + id)
        if (!id) return
        try {
    ...
    app.action('action_change_report_status', async ({ ack, body, payload, client }) => {
        ...
        console.log(`change report status, id: ${id}, status: ${status}`)
        if (!id) return
